...
Vertex2 centerScreen(framebuffer.getWidth() / 2, framebuffer.getHeight() / 2);

void point(Vertex2 punto) {
    if (punto.x >= 0 && punto.x < framebuffer.getWidth() && punto.y >= 0 && punto.y < framebuffer.getHeight()) {
...
void drawPolygon(const std::vector<Vertex2>& vertices) {
    size_t tamano = vertices.size();
    for (size_t i = 0; i < tamano; i++) {
        const Vertex2& vertex = vertices[i];
        if (i == tamano - 1) {
            const Vertex2& vertex2 = vertices[0];
            linea(vertex, vertex2);
        } else {
            const Vertex2& vertex2 = vertices[i + 1];
            linea(vertex, vertex2);
        }
    }
}
...
void fillBucket(int x, int y, Color targetColor, Color fillColor) {
    ...
    Color currentC = framebuffer.getPixel(x, y);
...
void fillPolygon(const std::vector<Vertex2>& vertices, Color fillColor, Color strokeColor, Color backgroundColor) {
    Vertex2 pointInPolygon = getRandomPointInsidePolygon(vertices);
    fillBucket(pointInPolygon.x, pointInPolygon.y, backgroundColor, fillColor, strokeColor);
}
